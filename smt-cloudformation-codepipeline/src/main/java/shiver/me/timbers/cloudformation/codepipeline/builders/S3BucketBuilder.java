...
public class S3BucketBuilder implements Builder<Pipeline, PipelineConfig> {

    @Override
    public void apply(Pipeline pipeline, PipelineConfig config) {
        final BucketResource bucketResource = new BucketResource(config.getResourceName() + "S3Bucket")
            .withProperties(
                new Bucket()
